...
if (!isset($data->id) || !isset($data->program_title) || !isset($data->date_and_time_start) || !isset($data->date_and_time_end) || !isset($data->place) || !isset($data->program_details) || !isset($data->program_lead) || !isset($data->program_members) || !isset($data->participants) || !isset($data->program_flow) || !isset($data->additional_details) || !isset($data->partners) || !isset($data->banner) || !isset($data->upload_files)) :
    echo json_encode([
        'success' => 0,
    ...
    exit;
elseif (empty(trim($data->id)) || empty(trim($data->program_title)) || empty(trim($data->date_and_time_start)) || empty(trim($data->date_and_time_end)) || empty(trim($data->place)) || empty(trim($data->program_details)) || empty(trim($data->program_lead)) || empty(trim($data->program_members)) || empty(trim($data->participants)) || empty(trim($data->program_flow)) || empty(trim($data->additional_details)) || empty(trim($data->partners)) || empty(trim($data->banner)) || empty(trim($data->upload_files))) :
    echo json_encode([
        'success' => 0,
